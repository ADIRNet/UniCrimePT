...
class PromptNet(nn.Module):
    def __init__(self, args):
        ...
        self.if_weather_in_day = args.if_time_in_day
        self.if_poi_in_week = args.if_day_in_week
        ...
        # temporal embeddings
        if self.if_weather_in_day:
            self.time_in_day_emb = nn.Linear(6, self.temp_dim_tid, bias=True)
            # nn.Embedding(288+1, self.temp_dim_tid)
        if self.if_poi_in_week:
            self.day_in_week_emb = nn.Linear(3, self.temp_dim_diw, bias=True)
            # nn.Embedding(7+1, self.temp_dim_diw)

        self.time_series_emb_layer = nn.Linear(self.input_len, self.embed_dim, bias=True)

        self.hidden_dim = self.embed_dim+self.node_dim * \
            int(self.if_spatial)+self.temp_dim_tid*int(self.if_poi_in_week) + \
            self.temp_dim_diw*int(self.if_weather_in_day)
        self.mlp = nn.Sequential(
            nn.Linear(self.input_base_dim, 128),
            nn.ReLU(),
            nn.Linear(128, self.temp_dim_tid * 4)
        )
        # Base
    ...
    def forward(self, history_data, source2, batch_seen=None, nadj=None, lpls=None, useGNN=False):
        ...
        if self.if_weather_in_day:
            t_i_d_data = source2[:, 0, :, self.input_base_dim:self.input_base_dim+6]
            time_in_day_emb = self.time_in_day_emb(t_i_d_data[:, :])
        else:
            time_in_day_emb = None
        if self.if_poi_in_week:
            d_i_w_data = source2[:, 0, :, self.input_base_dim+6:self.input_base_dim+9]
            # # 更改成缺失特征：
            # d_i_w_data[:, :, -1] = 0
        ...
        x_prompt = hidden.transpose(1, 3)
        x_prompt = F.normalize(x_prompt, dim=-1)
        x_prompt = x_prompt + input_data[..., 0:self.input_base_dim]
        return x_prompt
        # concate all embeddings
        # hidden = torch.cat([time_series_emb] + node_emb + tem_emb, dim=-1).transpose(1, 3)
        #
