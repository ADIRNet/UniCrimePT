...
eps = 1.0e-6
# ************************************************************************* #
 #1km时候全样本batch是20， 小样本是200
device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
args = parse_args(device)
args.mode = 'eval'  # 可选：pretrain, eval, ori, test
args.dataset_test = 'NEW_Theft'  # 可选：CHI_Theft, NEW_Theft, NYC_Theft, PHI_Theft LOS_Theft
...
args.load_pretrain_path = 'STGCN_NYC_Theft.pth_ep0_EL1.14_PL1.14.pth'  # 1km
# args.load_pretrain_path = 'STGCN_NYC_Theft.pth_ep87_EL0.83_PL0.83.pth' # 2km
...
if args.mode == 'pretrain':
    ...
    eval_x_tst_dict2, eval_y_tst_dict2, _ = define_dataloder(stage='eval', val_ratio=0.25, test_ratio=0.5,
                                                                         args=args)
    eval_x_tst_dict3, eval_y_tst_dict3, _ = define_dataloder(stage='eval', val_ratio=0.5, test_ratio=0.75,
                                                                         args=args)
    eval_x_tst_dict4, eval_y_tst_dict4, _ = define_dataloder(stage='eval', val_ratio=0.75, test_ratio=1,
                                                                         args=args)
