...
args.mode = 'ori'  # 可选：pretrain, eval, ori, test
args.dataset_test = 'NYC_Theft'  # 可选：CHI_Theft, NEW_Theft, NYC_Theft, PHI_Theft LOS_Theft
args.dataset_use = ['CHI_Theft']  # 可选：'NYC_Theft', 'CHI_Theft'
args.model = 'TGCN'  # 可选模型列表
args.load_pretrain_path = 'STGCN_PHI_Theft.pth_ep0_EL0.75_PL0.75.pth'
# args.load_pretrain_path = 'STGCN_NEW_Theft.pth_ep94_EL0.64_PL0.64.pth'
...
def infoNCEloss():
    def loss(q):
        T = 0.3
        gaussian_samples = torch.normal(mean=0.0, std=1.0, size=q.shape).to(q.device)
        pos_sim = torch.sum(torch.mul(q, gaussian_samples), dim=-1)
        neg_sim = torch.matmul(q, q.transpose(-1, -2))
        pos = torch.exp(torch.div(pos_sim, T))
        neg = torch.sum(torch.exp(torch.div(neg_sim, T)), dim=-1)
        denominator = neg + pos
...
if args.model == 'GWN' or args.model == 'MTGNN' or args.model == 'STFGNN' or args.model == 'STGODE' or args.model == 'DMSTGCN':
    seed_mode = True   # for quick running
else:
    seed_mode = True
...
if args.mode == 'pretrain':
    ...
    eval_x_tst_dict, eval_y_tst_dict, eval_scaler_dict = define_dataloder_test(stage='eval', args=args)
    eval_train_loader = get_val_tst_dataloader(eval_x_tst_dict, eval_y_tst_dict, args, shuffle=True)
    eval_val_loader = get_val_tst_dataloader(eval_x_tst_dict, eval_y_tst_dict, args, shuffle=False)
...
if args.mode == 'pretrain':
    ...
    if args.mode == 'eval':
        ...
        # 加载预训练参数
        pretrained_dict = torch.load(os.path.join(load_dir, args.load_pretrain_path))
        ...
        #             print(f"[WARN] Shape mismatch for {k}: pretrain {v.shape}, model {model_dict[k].shape}")
        #
        #             # 情况 1: 预训练小 -> 复制 + 补零
        #             if v.shape[-1] < model_dict[k].shape[-1]:
        #                 pad_size = model_dict[k].shape[-1] - v.shape[-1]
        #                 pad = torch.zeros(*v.shape[:-1], pad_size, device=v.device)
        #                 v = torch.cat([v, pad], dim=-1)
        #
        #             # 情况 2: 预训练大 -> 截断
        #             elif v.shape[-1] > model_dict[k].shape[-1]:
        #                 v = v[..., :model_dict[k].shape[-1]]
        #
        #         model_dict[k] = v
