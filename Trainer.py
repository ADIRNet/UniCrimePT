...
from lib.metrics import All_Metrics, nb_zeroinflated_draw
...
class Trainer(object):
    def __init__(self, model, loss, loss_ssl, optimizer, x_trn_dict, y_trn_dict, A_dict, lpls_dict, eval_train_dataloader,
                       eval_val_dataloader, eval_test_dataloader1, eval_test_dataloader2, eval_test_dataloader3, eval_test_dataloader4, scaler_dict, eval_scaler_dict,
        ...
        self.eval_test_loader1 = eval_test_dataloader1
        self.eval_test_loader2 = eval_test_dataloader2
        self.eval_test_loader3 = eval_test_dataloader3
        self.eval_test_loader4 = eval_test_dataloader4
        ...
        for epoch in tqdm(range(self.args.pretrain_epochs)):
            ...
            if epoch % 1 == 0:
                print(
                    ...
                    epoch + 1, self.args.pretrain_epochs, round(loss, 2), round(loss_pred, 2), round(loss_ssl, 4),
    ...
    def train_eval(self):
        ...
        F1_macro1, recall_macro1 = self.eval_test(self.model, self.args, self.A_dict, self.lpls_dict, self.eval_test_loader1, self.eval_scaler_dict[self.args.dataset_test], self.logger)
        F1_macro2, recall_macro2 = self.eval_test(self.model, self.args, self.A_dict, self.lpls_dict, self.eval_test_loader2, self.eval_scaler_dict[self.args.dataset_test], self.logger)
        F1_macro3, recall_macro3 = self.eval_test(self.model, self.args, self.A_dict, self.lpls_dict, self.eval_test_loader3, self.eval_scaler_dict[self.args.dataset_test], self.logger)
        F1_macro4, recall_macro4 = self.eval_test(self.model, self.args, self.A_dict, self.lpls_dict, self.eval_test_loader4, self.eval_scaler_dict[self.args.dataset_test], self.logger)
        # save_path = os.path.join(r"/opt/data/shanmx_projects/STNE_visual/loss/few_1km",
        #                          "PHI_all_prompt_loss.pt")  # _STGCN
        # torch.save(val_loss_list, save_path)
        print('final_result', (F1_macro1+ F1_macro2 +F1_macro3+F1_macro4)/4, (recall_macro1+ recall_macro2 +recall_macro3+recall_macro4)/4)
    ...
    def eval_trn_eps(self):
        ...
        for batch_idx, (data, target) in enumerate(self.eval_train_loader):
            ...
            if self.args.mode == 'eval':
                loss = loss_pred + 1 * self.loss_ssl(q, q)
            else:
                loss = loss_pred
            self.optimizer.zero_grad()
            loss.backward()
    ...
    def eval_test(model, args, A_dict, lpls_dict, data_loader, scaler, logger, path=None):
        ...
        y_true = []
        n, p, pi = [], [], []
        # all_features = []  # 用来存储所有 batch 的特征
        # all_prompt = []  # 用来存储所有 batch 的特征
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(data_loader):
                ...
                val_pred, val_prompt = model(data, data, args.dataset_test, batch_seen=None, nadj=nadj, lpls=lpls, useGNN=True)
                # data_ori = data.contiguous().view(data.size(0), -1).detach().cpu()
                # data_p = val_prompt.contiguous().view(val_prompt.size(0), -1).detach().cpu()
                # all_features.append(data_ori)
                # all_prompt.append(data_p)

                # output = (1 - val_pred[..., 2]) * (
                #         val_pred[..., 0] / val_pred[..., 1] - val_pred[..., 0])
                label = target[..., 0]
                y_true.append(label)
                y_pred.append(val_pred[..., 0])
                # pi.append(1 - val_pred[..., 2])
                # p.append(1 - val_pred[..., 1])
                # n.append(1 - val_pred[..., 0])
        if not args.real_value:
            y_true = scaler.inverse_transform(torch.cat(y_true, dim=0))
            y_pred = scaler.inverse_transform(torch.cat(y_pred, dim=0))
            # pi = torch.cat(pi, dim=0).cpu().numpy()
            # p = torch.cat(p, dim=0).cpu().numpy()
            # n = torch.cat(n, dim=0).cpu().numpy()
        ...
        for t in range(y_true.shape[1]):
            mae, rmse, recall_macro, F1_macro, recall_micro = All_Metrics(y_pred[:, t, ...], y_true[:, t, ...],
                                                args.mae_thresh, args.mape_thresh, y_pred[:, t, ...])
            logger.info("Horizon {:02d}, MAE: {:.2f}, RMSE: {:.2f}, F1_macro: {:.4f}, recall_macro: {:.3f}".format(
                t + 1, mae, rmse, F1_macro, recall_macro))
        mae, rmse, recall_macro, F1_macro, recall_micro = All_Metrics(y_pred, y_true, args.mae_thresh, args.mape_thresh, y_pred)
        #
        # samples = nb_zeroinflated_draw(n, p, pi, num_samples=100)  # shape: (100, ...)
        # lower = np.percentile(samples, 5, axis=0)
        # upper = np.percentile(samples, 95, axis=0)
        #
        # picp = np.mean((y_true.cpu().numpy() >= lower) & (y_true.cpu().numpy() <= upper))
        # mpiw = np.mean(upper - lower)

        logger.info("Average Horizon, MAE: {:.2f}, RMSE: {:.2f}, F1_macro: {:.3f}, recall_macro: {:.3f}".format(
                    mae, rmse, F1_macro, recall_macro))
        # # 拼接所有 batch 的结果，得到 (num_samples, feature_dim)
        # all_features = torch.cat(all_features, dim=0)
        #
        # # 保存到文件，方便后续用 sklearn 的 TSNE
        # save_path = os.path.join("..", "features.pt")
        # torch.save(all_features, save_path)
        # all_prompt = torch.cat(all_prompt, dim=0)
        # save_path = os.path.join("..", "LOS_all_prompt1.pt")
        # torch.save(all_prompt, save_path)
        return F1_macro, recall_macro
