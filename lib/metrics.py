...
def nb_zeroinflated_draw(n, p, pi, num_samples=100):
    """
    Vectorized sampling from Zero-Inflated Negative Binomial (ZINB).
    n, p, pi: shape (...), broadcastable to common shape
    Returns: samples of shape (num_samples, ...)
    """
    shape = n.shape
    # Expand to (num_samples, ...)
    n_expand = np.broadcast_to(n, (num_samples,) + shape)
    p_expand = np.broadcast_to(p, (num_samples,) + shape)
    pi_expand = np.broadcast_to(pi, (num_samples,) + shape)

    # 先决定是否是“零”
    bern_mask = np.random.binomial(n=1, p=1 - pi_expand)  # 1表示使用NB，0表示是0
    # 采样负二项分布
    nb_sample = np.random.negative_binomial(n=n_expand, p=p_expand)
    sample = nb_sample * bern_mask
    return sample  # shape: (num_samples, ...)





def F1_macro_np(pred, true):
    """
    pred, true: numpy arrays of same shape
    Apply threshold: pred > 1 → 1, else 0; true >= 1 → 1, else 0
    Output: macro F1 score
    """
    pred_bin = (pred < 0.01).astype(int)
    true_bin = (true >= 1).astype(int)
    return f1_score(true_bin.flatten(), pred_bin.flatten(), average='macro'), recall_score(true_bin.flatten(), pred_bin.flatten(), average='macro'), recall_score(true_bin.flatten(), pred_bin.flatten(), average='micro')

def F1_macro_torch(pred, true):
    """
    pred, true: torch tensors of same shape
    Apply threshold: pred > 1 → 1, else 0; true >= 1 → 1, else 0
    Output: macro F1 score
    """
    pred_bin = (torch.sigmoid(pred) > 0.5).int().cpu().numpy()
    true_bin = (true >= 1).int().cpu().numpy()
    return f1_score(true_bin.flatten(), pred_bin.flatten(), average='macro'), recall_score(true_bin.flatten(), pred_bin.flatten(), average='macro'), recall_score(true_bin.flatten(), pred_bin.flatten(), average='micro')
...
def All_Metrics(pred, true, mask1, mask2, pi):
    ...
    if type(pred) == np.ndarray:
        ...
        F1_macro, recall_macro, recall_micro = F1_macro_np(pi, true)
        # corr = 0
        ...
        F1_macro, recall_macro, recall_micro = F1_macro_torch(pi, true)
        corr = CORR_torch(pred, true, mask1)
        #pnbi = PNBI_torch(pred, true, mask1)
        #opnbi = oPNBI_torch(pred, true, mask2)
    else:
        raise TypeError
    return mae, rmse, recall_macro, F1_macro, recall_micro
